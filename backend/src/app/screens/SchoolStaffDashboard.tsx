...
import { MobilePageWrapper } from '../components/PageWrapper';
...
export function SchoolStaffDashboard() {
  ...
  const [filterSortState, setFilterSortState] = useState<FilterOptions>({
    sortBy: 'urgency',
    categories: [],
  });
  ...
  const pendingCount = useMemo(() => 
    issues.filter((i) => i.status === 'acknowledged' || i.status === 'in_progress').length,
    [issues]
  );
  ...
  const getFilteredIssues = (): Issue[] => {
    ...
    switch (activeFilter) {
      case 'critical':
        filtered = filtered.filter((i) => i.ai_urgency_score >= 90); // Changed from urgency_score
        break;
      case 'pending':
  ...
  const handleFilterApply = (newState: FilterOptions) => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#F8F9FA]">
      ...
      >
        <span className="material-symbols-rounded" style={{ fontSize: '24px', color: '#0D1B2A', fontVariationSettings: "'FILL' 0, 'wght' 600" }}>
          add
        </span>
      </motion.button>
