...
import { ArrowLeft, Edit2, ChevronRight, Briefcase, BookOpen, Shield } from 'lucide-react';
...
import { useCurrentUser, updateUserProfile } from '../hooks/useCurrentUser';
...
export function SettingsStaff() {
  ...
  useEffect(() => {
    if (user) {
      setProfileData({
        ...
        schoolId: user.school?.udise_code || '',
        schoolName: user.school?.name || '',
        contactNumber: user.phone || '',
        whatsappNumber: user.phone || '', // Can be separate field if needed
      });
    }
  }, [user]);
  ...
  // Derived flags
  const isPrincipal = location.pathname.includes('/principal') || user?.role === 'principal';
  // Derive the user's unique ID from their email (e.g. "pri001@setu.edu.in" → "PRI001")
  const uniqueId = user?.email ? user.email.split('@')[0].toUpperCase() : (profileData.schoolId || '');
        ...
        >
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              {/* Chips row: role-aware */}
              <div className="flex flex-wrap gap-2 mb-3">
                {isPrincipal ? (
                  /* Principal: show a single "Principal" role chip */
                  <span
                    style={{
                      display: 'inline-flex',
                      alignItems: 'center',
                  ...
                  /* Staff: show designation + subject chips */
                  <>
                    <span
                      style={{
                        display: 'inline-flex',
              ...
              {/* ID row — unique_id for principal, UDISE code for staff */}
              <p
                style={{
                  fontFamily: 'Noto Sans',
                  fontSize: '13px',
              ...
              >
                ...
                {isPrincipal ? uniqueId : profileData.schoolId}
              </p>
            </div>
          </div>
        </div>
